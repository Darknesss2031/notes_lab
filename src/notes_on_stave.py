...
from .tools import ScoreLabel
from .stats import StatsRepository
from datetime import datetime


notes_for_gen = ["b2", "a2", "g2", "f2", "e2", "d2", "c2",
                 "b1", "a1", "g1", "f1", "e1", "d1", "c1"]
notes_bass = ["d0", "c0", "b1", "a1", "g1", "f1", "e1",
              "d1", "c1", "b2", "a2", "g2", "f2", "e2"]
notes_height = {}
...
class Line:
    ...
    def __init__(self, color, pos, width, screen):
        """Initialise function for the line class."""
        self.pos = pos.copy()
        self.width = width
        self.color = color
        self.screen = screen
    ...
    def draw(self):
        ...
        pygame.draw.line(self.screen, self.color, self.pos,
                         (self.pos[0]+self.width, self.pos[1]), 2)

    def move(self, delta):
        """Call to move the line."""
...
class Note:
    ...
    def __init__(self, x, y, screen, name, icon="note.png",
                 extra=False, underline=False):
...
class Stave:
    ...
    def __init__(self, width, pos, screen):
        """Initialise function for stave class."""
        self.screen = screen
        self.width = width
        self.pos = pos
        self.lines = []
        ...
        for i in range(3, 12):
            self.lines.append(Line(colors[(i + 1) % 2], off_pos,
...
class GameProcess:
    ...
    def __init__(self, screen, maxscore=30, alter=False):
        """Initialise function for class of game process."""
        self.screen = screen
    ...
    def end_game(self):
        ...
        self.STOP = True
        self.ticker.join()
        self.stats.add_game("Notes on stave", self.correct,
                            self.maxscore, datetime.now().strftime("%d-%m-%Y"))

    def choose_icon(self, name, alter, oct):
        """Call to choose the icon of note."""
        return "note.png"
    ...
    def generate_note(self):
        ...
        extra = True if (name.startswith("a2") or
                         name.startswith("c1")) else False
        note = Note(400, notes_height[name], self.screen,
                    name, icon, extra, underline)
        self.stave.notes.append(note)

    def change_mode(self):
        """Call to change the mode of stave."""
    ...
    def process_key(self, key):
        ...
        if self.GAME_MODE == "BASS":
            name = (notes_bass[notes_for_gen.index(name.rstrip('#'))] +
